...
class BreathingEngine: ObservableObject {
    ...
    private func beginPhase(_ newPhase: BreathPhase) {
        phase = newPhase
        phaseDuration = Double(selectedPreset.duration(for: newPhase))
    ...
    private func advancePhase(spillMs: Int) {
        ...
        phase = next
        phaseDuration = Double(preset.duration(for: next))
    ...
    private func restoreLastPreset() {
        guard let savedId = UserDefaults.standard.string(forKey: lastPresetKey),
              let preset = builtInPresets.first(where: { $0.id == savedId })
        else { return }
        selectedPreset = preset
    }
}
